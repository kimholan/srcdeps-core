...
public class DigestOutputStream extends OutputStream {
    ...
    public void write(byte[] b) throws IOException {
        if (log.isTraceEnabled()) {
            log.trace("srcdeps updates digest with "+ SrcdepsCoreUtils.bytesToHexString(b));
        }
        write(b, 0, b.length);
    }
    ...
    public void write(byte[] input, int offset, int len) throws IOException {
        if (log.isTraceEnabled()) {
            log.trace("srcdeps updates digest with "+ SrcdepsCoreUtils.bytesToHexString(input, offset, len));
        }
    ...
    public void write(int b) throws IOException {
        if (log.isTraceEnabled()) {
            log.trace("srcdeps updates digest with "+ SrcdepsCoreUtils.bytesToHexString(new byte[] {(byte) b}));
        }
